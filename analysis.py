...
def analyze_results(results_path, mode):
    """Lê todos os CSVs de um diretório, calcula média e desvio padrão."""
    ...
    # Agrupa por modelo e calcula a média e desvio padrão para colunas numéricas
    ...
    # Achata os nomes das colunas (ex: de ('duration_seconds', 'mean') para 'duration_seconds_mean')
    stats_df.columns = ['_'.join(col).strip() for col in stats_df.columns.values]
    stats_df = stats_df.add_suffix(f'_{mode}') # Adiciona sufixo do modo (ex: _CC_MODE_OFF)
...
def main():
    ...
    cols_order = [
        ...
        'duration_seconds_mean_ON', 'duration_seconds_std_ON', 'duration_seconds_overhead_%',
        'gpu_energy_kWh_mean_OFF', 'gpu_energy_kWh_std_OFF', 
        'gpu_energy_kWh_mean_ON', 'gpu_energy_kWh_std_ON', 'gpu_energy_kWh_overhead_%',
